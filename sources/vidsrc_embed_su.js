...
function embedUrl(tmdbId, type, season, episode) {
  if (type === 'movie') return BASE + '/embed/movie/{tmdb}'.replace('{tmdb}', tmdbId);
  return BASE + '/embed/tv/{tmdb}/{season}/{episode}'.replace('{tmdb}', tmdbId).replace('{season}', season || 1).replace('{episode}', episode || 1);
}
