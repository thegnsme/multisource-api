 ...
 *   5. Decode base64/hex URLs in scripts
 *   6. Fetch known JSON API endpoints
 *   7. Evaluate JavaScript expressions for dynamic URLs
 *   8. Check for blob: URLs and MediaSource patterns
 */

const { smartFetch, parseM3U8 } = require("./antidetect");
...
function isVideoUrl(url) {
	...
	if (/\.(js|css|png|jpg|svg|ico|woff|ttf|php|html?)\b/i.test(path))
		return false;
...
function extractStreams(html) {
	...
	// Strategy 7: Extract from JSON-like objects in script tags
	const scriptRegex = /<script[^>]*>([\s\S]*?)<\/script>/gi;
	while ((match = scriptRegex.exec(html)) !== null) {
		const scriptContent = match[1];
		// Look for JSON objects with stream URLs
		const jsonRegex = /\{[^{}]*(?:m3u8|mp4)[^{}]*\}/gi;
		let jsonMatch;
	...
	// Strategy 8: Extract from window/document object assignments
	const windowPatterns = [
		/window\.\w+\s*=\s*["']([^"']+\.(?:m3u8|mp4)[^"']*)["']/gi,
		/document\.\w+\s*=\s*["']([^"']+\.(?:m3u8|mp4)[^"']*)["']/gi,
		/self\.\w+\s*=\s*["']([^"']+\.(?:m3u8|mp4)[^"']*)["']/gi,
	];
	...
	// Strategy 9: Extract from atob() decoded strings
	const atobRegex = /atob\(["']([A-Za-z0-9+/=]+)["']\)/g;
	while ((match = atobRegex.exec(html)) !== null) {
		try {
			...
			// Also check for m3u8 in decoded content
			const innerM3u8 = decoded.match(
				/https?:\/\/[^\s"'<>]+\.m3u8[^\s"'<>]*/gi,
			);
			if (innerM3u8) {
				for (const m3u8 of innerM3u8) {
					addStream(m3u8.replace(/['")>\s]+$/g, ""));
				}
			}
		} catch {}
	}
...
/**
 * Extract streams from JSON data
 */
...
/**
 * Try to follow an iframe src URL recursively (max depth 3)
 */
async function followIframe(src, depth = 0, referer = "") {
	if (depth > 3 || !src) return [];
	// Make relative URLs absolute
	if (src.startsWith("//")) src = "https:" + src;
	if (src.startsWith("/")) {
		try {
			src = referer ? new URL(src, referer).href : src;
		} catch {
			return [];
		}
	}
	...
	const { data: html } = await smartFetch(src, {
		referer,
		timeout: 8000,
		rateLimit: false,
	});
	...
	const childIframes = extractIframes(html);
	for (const iframe of childIframes) {
		const childStreams = await followIframe(iframe, depth + 1, src);
...
async function scrapeEmbedSource({
	...
	if (apiUrl) {
		const apiResult = await smartFetch(apiUrl, {
			referer,
			timeout,
			responseType: "text",
		});
	...
	if (streams.length === 0) {
		const { data: html, status: httpStatus } = await smartFetch(embedUrl, {
			referer,
			timeout,
		});
